...
Demo = {
  ...
  init: function() {
    ...
    if (!Demo.el.canvas.getContext) // no support for canvas
      ...
      Demo.el.blocks.val(Demo.blocks.serialize(Demo.blocks.examples.current()));//Converting to string
  ...
  packer: function() {
    ...
    if (size == 'automatic') {
      ...
      heightOfTruck = parseInt(dims[2].split(" ")[0]);

      return new Packer(parseInt(dims[1]*50), parseInt(dims[0]*50));
    }
  },
  ...
  canvas: {
    ...
    rect:  function(x, y, w, h, color) {
      Demo.el.draw.fillStyle = "#c0c0c0";
      Demo.el.draw.fillRect(x + 0.5, y + 0.5, w, h);
      Demo.el.draw.fillStyle = "#202020";
      Demo.el.draw.beginPath();
      if(w==h){
        Demo.el.draw.arc(x + w/2, y+w/2, w/2, 0, 2*Math.PI);
      }
      else{
        Demo.el.draw.fillStyle = "#202020";
      }
      ...
      Demo.el.draw.fillText(string, x+w/4, y+w/4);
    },
  ...
  blocks: {
    ...
    deserialize: function(val) {
      ...
      for(i = 0 ; i < blocks.length ; i++) {
        ...
        if (block.length >= 2)
          result.push({w: parseInt(block[0]*50), h: parseInt(block[1]*50), num: (block.length == 2 ? 1 : parseInt(block[2])) });
      }
      var expanded = [];
